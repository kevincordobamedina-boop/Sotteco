    ...
    switch($_GET["op"]){
        ...
        case "update":
            ...
            break;
                                                                        ...
                                                                        /* TODO: Actualizamos el ticket a anulado y adicionamos una linea adicional */
        case "anulado":
            $iv_dec = substr(base64_decode($_POST["tick_id"]), 0, openssl_cipher_iv_length($cipher));
            $cifradoSinIV = substr(base64_decode($_POST["tick_id"]), openssl_cipher_iv_length($cipher));
            $decifrado = openssl_decrypt($cifradoSinIV, $cipher, $key, OPENSSL_RAW_DATA, $iv_dec);

            $ticket->update_ticket_anulado($decifrado);
            $ticket->insert_ticketdetalle_anulado($decifrado,$_SESSION["usu_id"]);

            $email->ticket_agendada($decifrado);
          //  $whatsapp->w_ticket_cerrado($decifrado);
        ...
        case "listar":
            ...
            foreach($datos as $row){
                ...
                if ($row["tick_estado"]=="anulado"){
                    $sub_array[] = '<span class="label label-pill label-success">anulado</span>';
                }
        ...
        /* TODO: Listado de tickets,formato json para Datatable JS, filtro avanzado*/                 /* TODO: Listado de tickets,formato json para Datatable JS, filtro avanzado*/         
        ...
        case "listar_filtro":
            ...
            foreach($datos as $row){
                ...
                if ($row["tick_estado"]=="Abierto"){
                    $sub_array[] = '<span class="label label-pill label-success">Abierto</span>';
                }elseif ($row["tick_estado"]=="anulado"){
                    $sub_array[] = '<span class="label label-pill label-danger">anulado</span>';
                }else{
                    $sub_array[] = '<a onClick="CambiarEstado('.$row["tick_id"].')"><span class="label label-pill label-danger">Cerrado</span><a>';
                }
                ...
                if($row["usu_asig"]==null){
                    ...
                    $datos1=$tecnico->get_tecnico_x_id($row["usu_asig"]);
                    foreach($datos1 as $row1){
                        $sub_array[] = '<span class="label label-pill label-success">'. $row1["usu_nom"].'</span>';
                    }
                }
        ...
        case "totalcerrado";
            ...
            break;
             ...
             case "totalanulado";
            $datos=$ticket->get_ticket_totalanulado();  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
