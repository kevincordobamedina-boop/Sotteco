<?php
    ...
    switch($_GET["op"]){
        ...
        case "guardaryeditar":
            $datos= $tecnico->get_tecnico_x_correo($_POST["usu_correo"]);
            if(count($datos)==0){
                if(empty($_POST["usu_id"])){
                    $tecnico->insert_tecnico($_POST["usu_nom"],$_POST["usu_ape"],$_POST["usu_correo"],$_POST["usu_pass"],$_POST["rol_id"],$_POST["usu_telf"]);
                    echo "1";
                } else {
                    $tecnico->update_tecnico($_POST["usu_id"],$_POST["usu_nom"],$_POST["usu_ape"],$_POST["usu_correo"],$_POST["usu_pass"],$_POST["rol_id"],$_POST["usu_telf"]);
                    echo "2";
                }
            }else{
                echo "0";
        ...
        case "listar":
            ...
            foreach($datos as $row){
                ...
                //}if ($row["rol_id"]=="2"){
                //    $sub_array[] = '<span class="label label-pill label-info">Tecnico</span>';
                }else{
                    $sub_array[] = '<span class="label label-pill label-info">Tecnico</span>';
                }
        ...
        case "eliminar":
            $tecnico->delete_tecnico($_POST["usu_id"]);
            break;

        /* TODO: Mostrar en formato JSON segun usu_id */
        case "mostrar";
            $datos=$tecnico->get_tecnico_x_id($_POST["usu_id"]);  
        ...
        case "total";
            $datos=$tecnico->get_tecnico_total_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
        ...
        case "totalabierto";
            $datos=$tecnico->get_tecnico_totalabierto_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
        ...
        /* TODO: Cantidad de Ticket Cerrados por tecnico en formato JSON */
        case "totalcerrado";
            $datos=$tecnico->get_tecnico_totalcerrado_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
        ...
        case "grafico";
            $datos=$tecnico->get_tecnico_grafico($_POST["usu_id"]);  
            echo json_encode($datos);
            break;

        /* TODO: Formato para llenar combo en formato HTML */
        case "combo";
            $datos = $tecnico->get_tecnico_x_rol();
            if(is_array($datos)==true and count($datos)>0){
        ...
        case "password":
            ...
            $tecnico->update_tecnico_pass($_POST["usu_id"],$textoCifrado);
            break;
